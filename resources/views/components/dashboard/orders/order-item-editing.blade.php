...
<script>
    ...
    // refresh on close when order was already closed
    modal.addEventListener('hide.bs.modal', () => {
        if (form.dataset.refreshOnClose !== undefined) {
            $dispatch('order-items-changed')
            $dispatch('order-meta-changed')
        }
    })
...
<form wire:submit="saveOrderItem" x-data="{itemFound: true, initialRequest: true, newItem: false}" id="editOrderItemForm" @if($order->status->orderClosed())data-refresh-on-close @endif>
    @if($order->status->orderClosed())
        <div class="modal-body" wire:key="order_cancelled_modal_body">
            <div class="alert alert-danger">
                Diese Bestellung ist bereits {{strtolower($order->status->getShortName())}} und kann daher nicht bearbeitet werden.
            </div>
        </div>
        <div class="modal-footer" wire:key="order_cancelled_modal_footer">
    ...
    @else
        <div class="modal-body position-relative">
            ...
            <div x-show="itemFound">
                ...
                <div class="row mb-3">
                    <div class="col">
                        <div class="form-check">
                            <input class="form-check-input @error('updateDeposit')is-invalid @enderror" type="checkbox" id="updateDeposit" wire:model="updateDeposit">
                ...
                @if($this->singleItemAmount === null)
                    {{-- Add some dummy space to prevent dialog height change after choosing an item. Otherwise, buttons will jump around which is bad for UX as modal is vertically centered. --}}
                    <div class="form-text">
                        &nbsp;
                    </div>
                @endif
